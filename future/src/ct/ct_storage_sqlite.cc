...
bool CtStorageSqlite::save_treestore(const Glib::ustring& file_path, const CtStorageSyncPending& syncPending, Glib::ustring& error)
{
    try
    {
        ...
        if (_pDb == nullptr)
        {
            ...
            // saving nodes
            gint64 sequence{0};
            CtTreeIter ct_tree_iter = _pCtMainWin->curr_tree_store().get_ct_iter_first();
            while (ct_tree_iter) {
                ++sequence;
        ...
        else
        {
            ...
            {
                CtTreeIter ct_tree_iter = _pCtMainWin->curr_tree_store().get_node_from_node_id(node_pair.first);
                                           ...
                                           const int start_offset, const int end_offset)
{
    ...
    {
        for (CtAnchoredWidget* pAnchoredWidget : ct_tree_iter->get_embedded_pixbufs_tables_codeboxes(std::make_pair(start_offset, end_offset)))
        {
