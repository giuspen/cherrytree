...
def config_file_load(dad):
    ...
    if os.path.isfile(cons.CONFIG_PATH):
        ...
        dad.curr_colors = {
            'f':Gdk.color_parse(cfg.get(section, "fg")) if cfg.has_option(section, "fg") else None,
            'b':Gdk.color_parse(cfg.get(section, "bg")) if cfg.has_option(section, "bg") else None,
            'n':Gdk.color_parse(cfg.get(section, "nn")) if cfg.has_option(section, "nn") else None}
        ...
        dad.special_chars = cfg.get(section, "special_chars") if cfg.has_option(section, "special_chars") else SPECIAL_CHARS_DEFAULT
        dad.selword_chars = cfg.get(section, "selword_chars") if cfg.has_option(section, "selword_chars") else SELWORD_CHARS_DEFAULT
        dad.chars_listbul = cfg.get(section, "chars_listbul") if cfg.has_option(section, "chars_listbul") else CHARS_LISTBUL_DEFAULT
        dad.chars_toc = cfg.get(section, "chars_toc") if cfg.has_option(section, "chars_toc") else CHARS_TOC_DEFAULT
...
def config_file_apply(dad):
    ...
    if dad.line_wrapping: dad.sourceview.set_wrap_mode(Gtk.WrapMode.WORD)
    else: dad.sourceview.set_wrap_mode(Gtk.WrapMode.NONE)
    ...
    dad.ui.get_widget("/ToolBar").set_style(Gtk.ToolbarStyle.ICONS)
...
def set_tree_path_and_cursor_pos(dad):
    ...
    if dad.node_path:
        ...
        if node_iter_to_focus:
            ...
            dad.sourceview.scroll_to_mark(dad.curr_buffer.get_insert(), cons.SCROLL_MARGIN, False, 0.5, 0.5)
...
def preferences_tab_text_n_code(dad, vbox_all_nodes, pref_dialog):
    ...
    label_tab_width = Gtk.Label(label=_("Tab Width"))
    adj_tab_width = Gtk.Adjustment(value=dad.tabs_width, lower=1, upper=10000, step_incr=1)
    spinbutton_tab_width = Gtk.SpinButton()
    spinbutton_tab_width.set_adjustment(adj_tab_width)
    spinbutton_tab_width.set_value(dad.tabs_width)
    hbox_tab_width.pack_start(label_tab_width, False, True, 0)
    hbox_tab_width.pack_start(spinbutton_tab_width, False, True, 0)
    ...
    label_wrapping_indent = Gtk.Label(label=_("Line Wrapping Indentation"))
    adj_wrapping_indent = Gtk.Adjustment(value=dad.wrapping_indent, lower=-10000, upper=10000, step_incr=1)
    spinbutton_wrapping_indent = Gtk.SpinButton()
    spinbutton_wrapping_indent.set_adjustment(adj_wrapping_indent)
    spinbutton_wrapping_indent.set_value(dad.wrapping_indent)
    hbox_wrapping_indent.pack_start(label_wrapping_indent, False, True, 0)
    hbox_wrapping_indent.pack_start(spinbutton_wrapping_indent, False, True, 0)
    ...
    label_space_around_lines = Gtk.Label(label=_("Vertical Space Around Lines"))
    adj_space_around_lines = Gtk.Adjustment(value=dad.space_around_lines, lower=-0, upper=255, step_incr=1)
    spinbutton_space_around_lines = Gtk.SpinButton()
    spinbutton_space_around_lines.set_adjustment(adj_space_around_lines)
    spinbutton_space_around_lines.set_value(dad.space_around_lines)
    hbox_space_around_lines.pack_start(label_space_around_lines, False, True, 0)
    hbox_space_around_lines.pack_start(spinbutton_space_around_lines, False, True, 0)
    hbox_relative_wrapped_space = Gtk.HBox()
    hbox_relative_wrapped_space.set_spacing(4)
    label_relative_wrapped_space = Gtk.Label(label=_("Vertical Space in Wrapped Lines"))
    adj_relative_wrapped_space = Gtk.Adjustment(value=dad.relative_wrapped_space, lower=-0, upper=100, step_incr=1)
    spinbutton_relative_wrapped_space = Gtk.SpinButton()
    spinbutton_relative_wrapped_space.set_adjustment(adj_relative_wrapped_space)
    ...
    vbox_text_editor.pack_start(checkbutton_spaces_tabs, False, True, 0)
    vbox_text_editor.pack_start(checkbutton_line_wrap, False, True, 0)
    vbox_text_editor.pack_start(hbox_wrapping_indent, False, True, 0)
    vbox_text_editor.pack_start(checkbutton_auto_indent, False, True, 0)
    vbox_text_editor.pack_start(checkbutton_line_nums, False, True, 0)
    vbox_text_editor.pack_start(hbox_space_around_lines, False, True, 0)
    vbox_text_editor.pack_start(hbox_relative_wrapped_space, False, True, 0)
    frame_text_editor = Gtk.Frame(label="<b>"+_("Text Editor")+"</b>")
    frame_text_editor.get_label_widget().set_use_markup(True)
    frame_text_editor.set_shadow_type(Gtk.ShadowType.NONE)
    align_text_editor = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    label_timestamp = Gtk.Label(label=_("Timestamp Format"))
    entry_timestamp_format = Gtk.Entry()
    entry_timestamp_format.set_text(dad.timestamp_format)
    button_strftime_help = Gtk.Button()
    button_strftime_help.set_image(Gtk.Image.new_from_stock(Gtk.STOCK_HELP, Gtk.IconSize.BUTTON))
    hbox_timestamp.pack_start(label_timestamp, False, True, 0)
    hbox_timestamp.pack_start(entry_timestamp_format, True, True, 0)
    hbox_timestamp.pack_start(button_strftime_help, False, True, 0)
    hbox_horizontal_rule = Gtk.HBox()
    hbox_horizontal_rule.set_spacing(4)
    label_horizontal_rule = Gtk.Label(label=_("Horizontal Rule"))
    entry_horizontal_rule = Gtk.Entry()
    entry_horizontal_rule.set_text(dad.h_rule)
    hbox_horizontal_rule.pack_start(label_horizontal_rule, False, True, 0)
    hbox_horizontal_rule.pack_start(entry_horizontal_rule, True, True, 0)
    hbox_special_chars = Gtk.HBox()
    hbox_special_chars.set_spacing(4)
    vbox_special_chars = Gtk.VBox()
    label_special_chars = Gtk.Label(label=_("Special Characters"))
    hbox_reset = Gtk.HBox()
    button_reset = Gtk.Button()
    button_reset.set_image(Gtk.Image.new_from_stock(Gtk.STOCK_UNDO, Gtk.IconSize.BUTTON))
    button_reset.set_tooltip_text(_("Reset to Default"))
    hbox_reset.pack_start(Gtk.Label(), True, True, 0)
    hbox_reset.pack_start(button_reset, False, True, 0)
    hbox_reset.pack_start(Gtk.Label(), True, True, 0)
    vbox_special_chars.pack_start(Gtk.Label(), False, True, 0)
    vbox_special_chars.pack_start(label_special_chars, False, True, 0)
    vbox_special_chars.pack_start(hbox_reset, False, True, 0)
    vbox_special_chars.pack_start(Gtk.Label(), False, True, 0)
    frame_special_chars = Gtk.Frame()
    frame_special_chars.set_size_request(-1, 80)
    frame_special_chars.set_shadow_type(Gtk.ShadowType.IN)
    scrolledwindow_special_chars = Gtk.ScrolledWindow()
    scrolledwindow_special_chars.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
    ...
    hbox_special_chars.pack_start(vbox_special_chars, False, True, 0)
    hbox_special_chars.pack_start(frame_special_chars, True, True, 0)
    hbox_selword_chars = Gtk.HBox()
    hbox_selword_chars.set_spacing(4)
    label_selword_chars = Gtk.Label(label=_("Chars to Select at Double Click"))
    entry_selword_chars = Gtk.Entry()
    entry_selword_chars.set_text(dad.selword_chars)
    hbox_selword_chars.pack_start(label_selword_chars, False, True, 0)
    hbox_selword_chars.pack_start(entry_selword_chars, True, True, 0)

    vbox_misc_all = Gtk.VBox()
    vbox_misc_all.set_spacing(2)
    vbox_misc_all.pack_start(hbox_timestamp, True, True, 0)
    vbox_misc_all.pack_start(hbox_horizontal_rule, True, True, 0)
    vbox_misc_all.pack_start(hbox_special_chars, True, True, 0)
    vbox_misc_all.pack_start(hbox_selword_chars, True, True, 0)
    frame_misc_all = Gtk.Frame(label="<b>"+_("Miscellaneous")+"</b>")
    frame_misc_all.get_label_widget().set_use_markup(True)
    frame_misc_all.set_shadow_type(Gtk.ShadowType.NONE)
    align_misc_all = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_misc_all.set_padding(3, 6, 6, 6)
    align_misc_all.add(vbox_misc_all)
    frame_misc_all.add(align_misc_all)

    vbox_all_nodes.pack_start(frame_text_editor, False, True, 0)
    ...
    def on_checkbutton_line_wrap_toggled(checkbutton):
        ...
        dad.sourceview.set_wrap_mode(Gtk.WrapMode.WORD if dad.line_wrapping else Gtk.WrapMode.NONE)
...
def preferences_tab_rich_text(dad, vbox_text_nodes, pref_dialog):
    ...
    label_spell_check_lang = Gtk.Label(label=_("Spell Check Language"))
    ...
    hbox_spell_check_lang.pack_start(label_spell_check_lang, False, True, 0)
    hbox_spell_check_lang.pack_start(combobox_spell_check_lang, True, True, 0)
    vbox_spell_check.pack_start(checkbutton_spell_check, False, True, 0)
    vbox_spell_check.pack_start(hbox_spell_check_lang, False, True, 0)
    frame_spell_check = Gtk.Frame(label="<b>"+_("Spell Check")+"</b>")
    frame_spell_check.get_label_widget().set_use_markup(True)
    frame_spell_check.set_shadow_type(Gtk.ShadowType.NONE)
    align_spell_check = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    radiobutton_rt_col_dark = Gtk.RadioButton.new_with_label_from_widget(radiobutton_rt_col_light, _("Dark Background, Light Text"))
    radiobutton_rt_col_custom = Gtk.RadioButton.new_with_label_from_widget(radiobutton_rt_col_light, _("Custom Background"))
    hbox_rt_col_custom = Gtk.HBox()
    hbox_rt_col_custom.set_spacing(4)
    colorbutton_text_bg = Gtk.ColorButton(color=Gdk.color_parse(dad.rt_def_bg))
    label_rt_col_custom = Gtk.Label(label=_("and Text"))
    colorbutton_text_fg = Gtk.ColorButton(color=Gdk.color_parse(dad.rt_def_fg))
    hbox_rt_col_custom.pack_start(radiobutton_rt_col_custom, False, True, 0)
    hbox_rt_col_custom.pack_start(colorbutton_text_bg, False, True, 0)
    hbox_rt_col_custom.pack_start(label_rt_col_custom, False, True, 0)
    hbox_rt_col_custom.pack_start(colorbutton_text_fg, False, True, 0)
    ...
    colorbutton_monospace_bg = Gtk.ColorButton(color=Gdk.color_parse(mono_color))
    hbox_monospace_bg = Gtk.HBox()
    hbox_monospace_bg.set_spacing(4)
    hbox_monospace_bg.pack_start(checkbutton_monospace_bg, False, True, 0)
    hbox_monospace_bg.pack_start(colorbutton_monospace_bg, False, True, 0)

    vbox_rt_theme.pack_start(radiobutton_rt_col_light, False, True, 0)
    vbox_rt_theme.pack_start(radiobutton_rt_col_dark, False, True, 0)
    vbox_rt_theme.pack_start(hbox_rt_col_custom, False, True, 0)
    vbox_rt_theme.pack_start(hbox_monospace_bg, False, True, 0)
    frame_rt_theme = Gtk.Frame(label="<b>"+_("Theme")+"</b>")
    frame_rt_theme.get_label_widget().set_use_markup(True)
    frame_rt_theme.set_shadow_type(Gtk.ShadowType.NONE)
    align_rt_theme = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    label_embfile_size = Gtk.Label(label=_("Embedded File Icon Size"))
    adj_embfile_size = Gtk.Adjustment(value=dad.embfile_size, lower=1, upper=1000, step_incr=1)
    spinbutton_embfile_size = Gtk.SpinButton()
    spinbutton_embfile_size.set_adjustment(adj_embfile_size)
    spinbutton_embfile_size.set_value(dad.embfile_size)
    hbox_embfile_size.pack_start(label_embfile_size, False, True, 0)
    hbox_embfile_size.pack_start(spinbutton_embfile_size, False, True, 0)
    ...
    label_limit_undoable_steps = Gtk.Label(label=_("Limit of Undoable Steps Per Node"))
    adj_limit_undoable_steps = Gtk.Adjustment(value=dad.limit_undoable_steps, lower=1, upper=10000, step_incr=1)
    spinbutton_limit_undoable_steps = Gtk.SpinButton()
    spinbutton_limit_undoable_steps.set_adjustment(adj_limit_undoable_steps)
    spinbutton_limit_undoable_steps.set_value(dad.limit_undoable_steps)
    hbox_misc_text.pack_start(label_limit_undoable_steps, False, True, 0)
    hbox_misc_text.pack_start(spinbutton_limit_undoable_steps, False, True, 0)

    vbox_misc_text = Gtk.VBox()
    vbox_misc_text.pack_start(checkbutton_rt_show_white_spaces, False, True, 0)
    vbox_misc_text.pack_start(checkbutton_rt_highl_curr_line, False, True, 0)
    vbox_misc_text.pack_start(checkbutton_codebox_auto_resize, False, True, 0)
    vbox_misc_text.pack_start(hbox_embfile_size, False, True, 0)
    vbox_misc_text.pack_start(checkbutton_embfile_show_filename, False, True, 0)
    vbox_misc_text.pack_start(hbox_misc_text, False, True, 0)
    frame_misc_text = Gtk.Frame(label="<b>"+_("Miscellaneous")+"</b>")
    frame_misc_text.get_label_widget().set_use_markup(True)
    frame_misc_text.set_shadow_type(Gtk.ShadowType.NONE)
    align_misc_text = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_misc_text.set_padding(3, 6, 6, 6)
    align_misc_text.add(vbox_misc_text)
    frame_misc_text.add(align_misc_text)

    vbox_text_nodes.pack_start(frame_spell_check, False, True, 0)
    vbox_text_nodes.pack_start(frame_rt_theme, False, True, 0)
    vbox_text_nodes.pack_start(frame_misc_text, False, True, 0)
    ...
    def on_radiobutton_rt_col_light_toggled(radiobutton):
        ...
        colorbutton_text_fg.set_color(Gdk.color_parse(cons.RICH_TEXT_LIGHT_FG))
        colorbutton_text_bg.set_color(Gdk.color_parse(cons.RICH_TEXT_LIGHT_BG))
        colorbutton_text_fg.set_sensitive(False)
        colorbutton_text_bg.set_sensitive(False)
    ...
    def on_radiobutton_rt_col_dark_toggled(radiobutton):
        if not radiobutton.get_active(): return
        colorbutton_text_fg.set_color(Gdk.color_parse(cons.RICH_TEXT_DARK_FG))
        colorbutton_text_bg.set_color(Gdk.color_parse(cons.RICH_TEXT_DARK_BG))
        colorbutton_text_fg.set_sensitive(False)
        colorbutton_text_bg.set_sensitive(False)
        on_colorbutton_text_fg_color_set(colorbutton_text_fg)
        on_colorbutton_text_bg_color_set(colorbutton_text_bg)
...
def preferences_tab_text(dad, vbox_text, pref_dialog):
    ...
    vbox_editor.pack_start(checkbutton_auto_smart_quotes, False, True, 0)

    frame_editor = Gtk.Frame(label="<b>"+_("Text Editor")+"</b>")
    frame_editor.get_label_widget().set_use_markup(True)
    frame_editor.set_shadow_type(Gtk.ShadowType.NONE)
    align_editor = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_editor.set_padding(3, 6, 6, 6)
    align_editor.add(vbox_editor)
    frame_editor.add(align_editor)

    vbox_text.pack_start(frame_editor, False, True, 0)
...
def preferences_tab_plain_text_n_code(dad, vbox_code_nodes, pref_dialog):
    ...
    label_style_scheme = Gtk.Label(label=_("Style Scheme"))
    ...
    hbox_style_scheme.pack_start(label_style_scheme, False, True, 0)
    hbox_style_scheme.pack_start(combobox_style_scheme, True, True, 0)
    ...
    vbox_syntax.pack_start(hbox_style_scheme, False, True, 0)
    vbox_syntax.pack_start(checkbutton_pt_show_white_spaces, False, True, 0)
    vbox_syntax.pack_start(checkbutton_pt_highl_curr_line, False, True, 0)

    frame_syntax = Gtk.Frame(label="<b>"+_("Text Editor")+"</b>")
    frame_syntax.get_label_widget().set_use_markup(True)
    frame_syntax.set_shadow_type(Gtk.ShadowType.NONE)
    align_syntax = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_syntax.set_padding(3, 6, 6, 6)
    align_syntax.add(vbox_syntax)
    frame_syntax.add(align_syntax)

    vbox_code_nodes.pack_start(frame_syntax, False, True, 0)
    ...
    button_reset_cmds.set_image(Gtk.Image.new_from_stock(Gtk.STOCK_UNDO, Gtk.IconSize.BUTTON))
    button_reset_cmds.set_tooltip_text(_("Reset to Default"))
    vbox_buttons = Gtk.VBox()
    vbox_buttons.pack_start(button_add, False, True, 0)
    vbox_buttons.pack_start(Gtk.Label(), True, True, 0)
    vbox_buttons.pack_start(button_reset_cmds, False, True, 0)
    ...
    button_reset_term.set_image(Gtk.Image.new_from_stock(Gtk.STOCK_UNDO, Gtk.IconSize.BUTTON))
    button_reset_term.set_tooltip_text(_("Reset to Default"))
    hbox_term_run.pack_start(entry_term_run, True, True, 0)
    hbox_term_run.pack_start(button_reset_term, False, True, 0)
    hbox_cmd_per_type = Gtk.HBox()
    hbox_cmd_per_type.pack_start(scrolledwindow, True, True, 0)
    hbox_cmd_per_type.pack_start(vbox_buttons, False, True, 0)

    label = Gtk.Label(label="<b>"+_("Command per Node/CodeBox Type")+"</b>")
    label.set_use_markup(True)
    vbox_codexec.pack_start(label, False, True, 0)
    vbox_codexec.pack_start(hbox_cmd_per_type, True, True, 0)
    label = Gtk.Label(label="<b>"+_("Terminal Command")+"</b>")
    label.set_use_markup(True)
    vbox_codexec.pack_start(label, False, True, 0)
    vbox_codexec.pack_start(hbox_term_run, False, True, 0)

    frame_codexec = Gtk.Frame(label="<b>"+_("Code Execution")+"</b>")
    frame_codexec.get_label_widget().set_use_markup(True)
    frame_codexec.set_shadow_type(Gtk.ShadowType.NONE)
    align_codexec = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    vbox_code_nodes.pack_start(frame_codexec, True, True, 0)
...
def preferences_tab_tree_2(dad, vbox_tree, pref_dialog):
    ...
    label_tree_nodes_names_width = Gtk.Label(label=_("Tree Nodes Names Wrapping Width"))
    adj_tree_nodes_names_width = Gtk.Adjustment(value=dad.cherry_wrap_width, lower=10, upper=10000, step_incr=1)
    spinbutton_tree_nodes_names_width = Gtk.SpinButton()
    spinbutton_tree_nodes_names_width.set_adjustment(adj_tree_nodes_names_width)
    spinbutton_tree_nodes_names_width.set_value(dad.cherry_wrap_width)
    hbox_tree_nodes_names_width.pack_start(label_tree_nodes_names_width, False, True, 0)
    hbox_tree_nodes_names_width.pack_start(spinbutton_tree_nodes_names_width, False, True, 0)
    ...
    label_nodes_on_node_name_header = Gtk.Label(label=_("Last Visited Nodes on Node Name Header"))
    adj_nodes_on_node_name_header = Gtk.Adjustment(value=dad.nodes_on_node_name_header, lower=0, upper=100, step_incr=1)
    spinbutton_nodes_on_node_name_header = Gtk.SpinButton()
    spinbutton_nodes_on_node_name_header.set_adjustment(adj_nodes_on_node_name_header)
    spinbutton_nodes_on_node_name_header.set_value(dad.nodes_on_node_name_header)
    hbox_nodes_on_node_name_header.pack_start(label_nodes_on_node_name_header, False, True, 0)
    hbox_nodes_on_node_name_header.pack_start(spinbutton_nodes_on_node_name_header, False, True, 0)

    vbox_misc_tree.pack_start(hbox_tree_nodes_names_width, False, True, 0)
    vbox_misc_tree.pack_start(checkbutton_tree_right_side, False, True, 0)
    vbox_misc_tree.pack_start(checkbutton_tree_click_focus_text, False, True, 0)
    vbox_misc_tree.pack_start(checkbutton_tree_click_expand, False, True, 0)
    vbox_misc_tree.pack_start(hbox_nodes_on_node_name_header, False, True, 0)
    frame_misc_tree = Gtk.Frame(label="<b>"+_("Miscellaneous")+"</b>")
    frame_misc_tree.get_label_widget().set_use_markup(True)
    frame_misc_tree.set_shadow_type(Gtk.ShadowType.NONE)
    align_misc_tree = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_misc_tree.set_padding(3, 6, 6, 6)
    align_misc_tree.add(vbox_misc_tree)
    frame_misc_tree.add(align_misc_tree)

    vbox_tree.pack_start(frame_misc_tree, False, True, 0)
...
def preferences_tab_tree_1(dad, vbox_tree, pref_dialog):
    ...
    radiobutton_tt_col_dark = Gtk.RadioButton.new_with_label_from_widget(radiobutton_tt_col_light, _("Dark Background, Light Text"))
    radiobutton_tt_col_custom = Gtk.RadioButton.new_with_label_from_widget(radiobutton_tt_col_light, _("Custom Background"))
    hbox_tt_col_custom = Gtk.HBox()
    hbox_tt_col_custom.set_spacing(4)
    colorbutton_tree_bg = Gtk.ColorButton(color=Gdk.color_parse(dad.tt_def_bg))
    label_tt_col_custom = Gtk.Label(label=_("and Text"))
    colorbutton_tree_fg = Gtk.ColorButton(color=Gdk.color_parse(dad.tt_def_fg))
    hbox_tt_col_custom.pack_start(radiobutton_tt_col_custom, False, True, 0)
    hbox_tt_col_custom.pack_start(colorbutton_tree_bg, False, True, 0)
    hbox_tt_col_custom.pack_start(label_tt_col_custom, False, True, 0)
    hbox_tt_col_custom.pack_start(colorbutton_tree_fg, False, True, 0)

    vbox_tt_theme.pack_start(radiobutton_tt_col_light, False, True, 0)
    vbox_tt_theme.pack_start(radiobutton_tt_col_dark, False, True, 0)
    vbox_tt_theme.pack_start(hbox_tt_col_custom, False, True, 0)
    frame_tt_theme = Gtk.Frame(label="<b>"+_("Theme")+"</b>")
    frame_tt_theme.get_label_widget().set_use_markup(True)
    frame_tt_theme.set_shadow_type(Gtk.ShadowType.NONE)
    align_tt_theme = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    radiobutton_node_icon_custom = Gtk.RadioButton.new_with_label_from_widget(radiobutton_node_icon_cherry, _("Use Selected Icon"))
    radiobutton_node_icon_none = Gtk.RadioButton.new_with_label_from_widget(radiobutton_node_icon_cherry, _("No Icon"))
    ...
    c_icon_button.set_image(Gtk.Image.new_from_stock(cons.NODES_STOCKS[dad.default_icon_text], Gtk.IconSize.BUTTON))
    c_icon_hbox = Gtk.HBox()
    c_icon_hbox.set_spacing(2)
    c_icon_hbox.pack_start(radiobutton_node_icon_custom, False, True, 0)
    c_icon_hbox.pack_start(c_icon_button, False, True, 0)

    vbox_nodes_icons.pack_start(radiobutton_node_icon_cherry, False, True, 0)
    vbox_nodes_icons.pack_start(c_icon_hbox, False, True, 0)
    vbox_nodes_icons.pack_start(radiobutton_node_icon_none, False, True, 0)
    vbox_nodes_icons.pack_start(checkbutton_aux_icon_hide, False, True, 0)
    frame_nodes_icons = Gtk.Frame(label="<b>"+_("Default Text Nodes Icons")+"</b>")
    frame_nodes_icons.get_label_widget().set_use_markup(True)
    frame_nodes_icons.set_shadow_type(Gtk.ShadowType.NONE)
    align_nodes_icons = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    radiobutton_nodes_startup_expand = Gtk.RadioButton.new_with_label_from_widget(radiobutton_nodes_startup_restore, _("Expand all Nodes"))
    radiobutton_nodes_startup_collapse = Gtk.RadioButton.new_with_label_from_widget(radiobutton_nodes_startup_restore, _("Collapse all Nodes"))
    ...
    vbox_nodes_startup.pack_start(radiobutton_nodes_startup_restore, False, True, 0)
    vbox_nodes_startup.pack_start(radiobutton_nodes_startup_expand, False, True, 0)
    vbox_nodes_startup.pack_start(radiobutton_nodes_startup_collapse, False, True, 0)
    vbox_nodes_startup.pack_start(checkbutton_nodes_bookm_exp, False, True, 0)
    ...
    frame_nodes_startup.set_shadow_type(Gtk.ShadowType.NONE)
    align_nodes_startup = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    vbox_tree.pack_start(frame_tt_theme, False, True, 0)
    vbox_tree.pack_start(frame_nodes_icons, False, True, 0)
    vbox_tree.pack_start(frame_nodes_startup, False, True, 0)
    ...
    def on_radiobutton_tt_col_light_toggled(radiobutton):
        if not radiobutton.get_active(): return
        colorbutton_tree_fg.set_color(Gdk.color_parse(cons.TREE_TEXT_LIGHT_FG))
        colorbutton_tree_bg.set_color(Gdk.color_parse(cons.TREE_TEXT_LIGHT_BG))
        colorbutton_tree_fg.set_sensitive(False)
        colorbutton_tree_bg.set_sensitive(False)
    ...
    def on_radiobutton_tt_col_dark_toggled(radiobutton):
        if not radiobutton.get_active(): return
        colorbutton_tree_fg.set_color(Gdk.color_parse(cons.TREE_TEXT_DARK_FG))
        colorbutton_tree_bg.set_color(Gdk.color_parse(cons.TREE_TEXT_DARK_BG))
        colorbutton_tree_fg.set_sensitive(False)
        colorbutton_tree_bg.set_sensitive(False)
        on_colorbutton_tree_fg_color_set(colorbutton_tree_fg)
        on_colorbutton_tree_bg_color_set(colorbutton_tree_bg)
...
def preferences_tab_fonts(dad, vbox_fonts, pref_dialog):
    ...
    image_text.set_from_stock(Gtk.STOCK_SELECT_FONT, Gtk.IconSize.MENU)
    image_code = Gtk.Image()
    image_code.set_from_stock(Gtk.STOCK_SELECT_FONT, Gtk.IconSize.MENU)
    image_tree = Gtk.Image()
    image_tree.set_from_stock('cherries', Gtk.IconSize.MENU)
    label_text = Gtk.Label(label=_("Text Font"))
    label_code = Gtk.Label(label=_("Code Font"))
    label_tree = Gtk.Label(label=_("Tree Font"))
    fontbutton_text = Gtk.FontButton.new_with_font(dad.text_font)
    fontbutton_code = Gtk.FontButton.new_with_font(dad.code_font)
    fontbutton_tree = Gtk.FontButton.new_with_font(dad.tree_font)
    ...
    frame_fonts.set_shadow_type(Gtk.ShadowType.NONE)
    align_fonts = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_fonts.set_padding(3, 6, 6, 6)
    align_fonts.add(table_fonts)
    frame_fonts.add(align_fonts)

    vbox_fonts.pack_start(frame_fonts, False, True, 0)
...
def preferences_tab_links(dad, vbox_links, pref_dialog):
    ...
    vbox_links_actions.pack_start(checkbutton_custom_weblink_cmd, False, True, 0)
    vbox_links_actions.pack_start(entry_custom_weblink_cmd, False, True, 0)
    vbox_links_actions.pack_start(checkbutton_custom_filelink_cmd, False, True, 0)
    vbox_links_actions.pack_start(entry_custom_filelink_cmd, False, True, 0)
    vbox_links_actions.pack_start(checkbutton_custom_folderlink_cmd, False, True, 0)
    vbox_links_actions.pack_start(entry_custom_folderlink_cmd, False, True, 0)

    frame_links_actions = Gtk.Frame(label="<b>"+_("Custom Actions")+"</b>")
    frame_links_actions.get_label_widget().set_use_markup(True)
    frame_links_actions.set_shadow_type(Gtk.ShadowType.NONE)
    align_links_actions = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    label_col_link_webs = Gtk.Label(label=_("To WebSite"))
    colorbutton_col_link_webs = Gtk.ColorButton(color=Gdk.color_parse(dad.col_link_webs))
    hbox_col_link_webs.pack_start(label_col_link_webs, False, True, 0)
    hbox_col_link_webs.pack_start(colorbutton_col_link_webs, False, True, 0)

    hbox_col_link_node = Gtk.HBox()
    hbox_col_link_node.set_spacing(4)
    label_col_link_node = Gtk.Label(label=_("To Node"))
    colorbutton_col_link_node = Gtk.ColorButton(color=Gdk.color_parse(dad.col_link_node))
    hbox_col_link_node.pack_start(label_col_link_node, False, True, 0)
    hbox_col_link_node.pack_start(colorbutton_col_link_node, False, True, 0)

    hbox_col_link_file = Gtk.HBox()
    hbox_col_link_file.set_spacing(4)
    label_col_link_file = Gtk.Label(label=_("To File"))
    colorbutton_col_link_file = Gtk.ColorButton(color=Gdk.color_parse(dad.col_link_file))
    hbox_col_link_file.pack_start(label_col_link_file, False, True, 0)
    hbox_col_link_file.pack_start(colorbutton_col_link_file, False, True, 0)

    hbox_col_link_fold = Gtk.HBox()
    hbox_col_link_fold.set_spacing(4)
    label_col_link_fold = Gtk.Label(label=_("To Folder"))
    colorbutton_col_link_fold = Gtk.ColorButton(color=Gdk.color_parse(dad.col_link_fold))
    hbox_col_link_fold.pack_start(label_col_link_fold, False, True, 0)
    hbox_col_link_fold.pack_start(colorbutton_col_link_fold, False, True, 0)
    ...
    frame_links_colors.set_shadow_type(Gtk.ShadowType.NONE)
    align_links_colors = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    label_anchor_size = Gtk.Label(label=_("Anchor Size"))
    adj_anchor_size = Gtk.Adjustment(value=dad.anchor_size, lower=1, upper=1000, step_incr=1)
    spinbutton_anchor_size = Gtk.SpinButton()
    spinbutton_anchor_size.set_adjustment(adj_anchor_size)
    spinbutton_anchor_size.set_value(dad.anchor_size)
    hbox_anchor_size.pack_start(label_anchor_size, False, True, 0)
    hbox_anchor_size.pack_start(spinbutton_anchor_size, False, True, 0)
    vbox_links_misc.pack_start(checkbutton_links_underline, False, True, 0)
    vbox_links_misc.pack_start(checkbutton_links_relative, False, True, 0)
    vbox_links_misc.pack_start(hbox_anchor_size, False, True, 0)

    frame_links_misc = Gtk.Frame(label="<b>"+_("Miscellaneous")+"</b>")
    frame_links_misc.get_label_widget().set_use_markup(True)
    frame_links_misc.set_shadow_type(Gtk.ShadowType.NONE)
    align_links_misc = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_links_misc.set_padding(3, 6, 6, 6)
    align_links_misc.add(vbox_links_misc)
    frame_links_misc.add(align_links_misc)

    vbox_links.pack_start(frame_links_actions, False, True, 0)
    vbox_links.pack_start(frame_links_colors, False, True, 0)
    vbox_links.pack_start(frame_links_misc, False, True, 0)
...
def preferences_tab_kb_shortcuts(dad, vbox_tool, pref_dialog):
    ...
    button_edit.set_image(Gtk.Image.new_from_stock(Gtk.STOCK_EDIT, Gtk.IconSize.BUTTON))
    button_edit.set_tooltip_text(_("Change Selected"))
    button_reset = Gtk.Button()
    button_reset.set_image(Gtk.Image.new_from_stock(Gtk.STOCK_UNDO, Gtk.IconSize.BUTTON))
    button_reset.set_tooltip_text(_("Reset to Default"))
    vbox_buttons.pack_start(button_edit, False, True, 0)
    vbox_buttons.pack_start(Gtk.Label(), True, True, 0)
    vbox_buttons.pack_start(button_reset, False, True, 0)
    hbox_main = Gtk.HBox()
    hbox_main.pack_start(scrolledwindow, True, True, 0)
    hbox_main.pack_start(vbox_buttons, False, True, 0)
...
["import", _("Import")],
    ...
    def edit_selected_tree_iter():
        ...
        radiobutton_kb_shortcut = Gtk.RadioButton.new_from_widget(radiobutton_kb_none)
        ...
        vbox = Gtk.VBox()
        hbox = Gtk.HBox()
        hbox.pack_start(radiobutton_kb_shortcut, True, True, 0)
        hbox.pack_start(ctrl_toggle, True, True, 0)
        hbox.pack_start(shift_toggle, True, True, 0)
        hbox.pack_start(alt_toggle, True, True, 0)
        hbox.pack_start(key_entry, True, True, 0)
        hbox.set_spacing(5)
        vbox.pack_start(radiobutton_kb_none, True, True, 0)
...
def preferences_tab_toolbar(dad, vbox_tool, pref_dialog):
    ...
    hbox = Gtk.HBox()
    vbox = Gtk.VBox()
    vbox.pack_start(button_add, False, True, 0)
    vbox.pack_start(button_remove, False, True, 0)
    vbox.pack_start(Gtk.Label(), True, True, 0)
    vbox.pack_start(button_reset, False, True, 0)
...
def preferences_tab_misc(dad, vbox_misc, pref_dialog):
    ...
    vbox_system_tray.pack_start(checkbutton_systray, False, True, 0)
    vbox_system_tray.pack_start(checkbutton_start_on_systray, False, True, 0)
    vbox_system_tray.pack_start(checkbutton_use_appind, False, True, 0)

    frame_system_tray = Gtk.Frame(label="<b>"+_("System Tray")+"</b>")
    frame_system_tray.get_label_widget().set_use_markup(True)
    frame_system_tray.set_shadow_type(Gtk.ShadowType.NONE)
    align_system_tray = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    spinbutton_autosave = Gtk.SpinButton()
    spinbutton_autosave.set_adjustment(adjustment_autosave)
    label_autosave = Gtk.Label(label=_("Minutes"))
    hbox_autosave.pack_start(checkbutton_autosave, False, True, 0)
    hbox_autosave.pack_start(spinbutton_autosave, False, True, 0)
    hbox_autosave.pack_start(label_autosave, False, True, 0)
    ...
    label_num_backups = Gtk.Label(label=_("Number of Backups to Keep"))
    adjustment_num_backups = Gtk.Adjustment(value=dad.backup_num, lower=1, upper=100, step_incr=1)
    spinbutton_num_backups = Gtk.SpinButton()
    spinbutton_num_backups.set_adjustment(adjustment_num_backups)
    spinbutton_num_backups.set_sensitive(dad.backup_copy)
    spinbutton_num_backups.set_value(dad.backup_num)
    hbox_num_backups.pack_start(label_num_backups, False, True, 0)
    hbox_num_backups.pack_start(spinbutton_num_backups, False, True, 0)
    vbox_saving.pack_start(hbox_autosave, False, True, 0)
    vbox_saving.pack_start(checkbutton_autosave_on_quit, False, True, 0)
    vbox_saving.pack_start(checkbutton_backup_before_saving, False, True, 0)
    vbox_saving.pack_start(hbox_num_backups, False, True, 0)
    ...
    frame_saving.set_shadow_type(Gtk.ShadowType.NONE)
    align_saving = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    vbox_misc_misc.pack_start(checkbutton_newer_version, False, True, 0)
    vbox_misc_misc.pack_start(checkbutton_word_count, False, True, 0)
    vbox_misc_misc.pack_start(checkbutton_reload_doc_last, False, True, 0)
    vbox_misc_misc.pack_start(checkbutton_mod_time_sentinel, False, True, 0)
    ...
    frame_misc_misc.set_shadow_type(Gtk.ShadowType.NONE)
    align_misc_misc = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    ...
    vbox_language.pack_start(combobox_country_language, True, True, 0)
    cell = Gtk.CellRendererText()
    ...
    frame_language.set_shadow_type(Gtk.ShadowType.NONE)
    align_language = Gtk.Alignment.new(*cons.XY_ALIGN_XY_SCALE)
    align_language.set_padding(3, 6, 6, 6)
    align_language.add(vbox_language)
    frame_language.add(align_language)

    vbox_misc.pack_start(frame_system_tray, False, True, 0)
    vbox_misc.pack_start(frame_saving, False, True, 0)
    vbox_misc.pack_start(frame_misc_misc, False, True, 0)
    vbox_misc.pack_start(frame_language, False, True, 0)
...
def dialog_preferences(dad):
    ...
    notebook.append_page(tabs_vbox_vec[0], Gtk.Label(label=_("Text and Code")))
    notebook.append_page(tabs_vbox_vec[1], Gtk.Label(label=_("Text")))
    notebook.append_page(tabs_vbox_vec[2], Gtk.Label(label=_("Rich Text")))
    notebook.append_page(tabs_vbox_vec[3], Gtk.Label(label=_("Plain Text and Code")))
    notebook.append_page(tabs_vbox_vec[4], Gtk.Label(label=_("Tree")+" 1"))
    notebook.append_page(tabs_vbox_vec[5], Gtk.Label(label=_("Tree")+" 2"))
    notebook.append_page(tabs_vbox_vec[6], Gtk.Label(label=_("Fonts")))
    notebook.append_page(tabs_vbox_vec[7], Gtk.Label(label=_("Links")))
    notebook.append_page(tabs_vbox_vec[8], Gtk.Label(label=_("Toolbar")))
    notebook.append_page(tabs_vbox_vec[9], Gtk.Label(label=_("Keyboard Shortcuts")))
    notebook.append_page(tabs_vbox_vec[10], Gtk.Label(label=_("Miscellaneous")))
