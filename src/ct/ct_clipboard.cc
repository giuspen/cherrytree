...
void CtClipboard::_cut_clipboard(Gtk::TextView* pTextView, CtCodebox* pCodebox)
{
    ...
    if (text_buffer->get_has_selection()) {
        ...
        if ((pCodebox or _pCtMainWin->curr_tree_iter().get_node_syntax_highlighting() != CtConst::RICH_TEXT_ID) and
            num_chars > 30000)
        {
                                       ...
                                       CtTableCommon* parentTable)
{
    ...
    if (parentTable) {
        ...
        if (is_column) {
            ...
            // row paste
            int insert_after = static_cast<int>(parentTable->current_row()) - 1;
            if (insert_after < 0) insert_after = 0;
